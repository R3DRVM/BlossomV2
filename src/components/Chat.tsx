...
export default function Chat({ selectedStrategyId, executionMode = 'auto', onRegisterInsertPrompt }: ChatProps) {
  ...
  useEffect(() => {
    ...
    const runAcceptance8 = async () => {
      ...
      // Helper to fund USDC (disabled - fundUsdc not available)
      ...
      // Test 5: Rest-of-portfolio with low USDC
      try {
        await reset();
        ...
        results.push({ 
          step: 5, 
          name: 'Rest-of-portfolio with low USDC blocks', 
          pass: test5Pass,
          reason: test5Pass ? undefined : 'Strategy not blocked when USDC is low' 
        });
      } catch (e: any) {
        results.push({ step: 5, name: 'Rest-of-portfolio with low USDC blocks', pass: false, reason: e.message });
      }
      ...
      try {
        ...
        results.push({ 
          step: 6, 
          name: 'Fund USDC increases balance', 
          pass: test6Pass,
          reason: test6Pass ? undefined : 'USDC balance not increased' 
        });
      } catch (e: any) {
        results.push({ step: 6, name: 'Fund USDC increases balance', pass: false, reason: e.message });
      }
