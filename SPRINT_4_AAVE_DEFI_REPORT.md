...
AAVE_REDACTED_ADDRESS=0x...  # Optional: REDACTED address on Sepolia (defaults to market config)
...
4. **Asset Support**: Currently supports REDACTED only. Additional assets can be added to `market.ts` `supportedAssets` array.
...
1. **User creates DeFi plan** → `executionRequest: { kind: 'lend', amountUsd: 100, asset: 'REDACTED' }`
