 ...
 *
 * Modes:
 *   small    - Minimal amounts (0.001 SOL, 1 REDACTED) for quick tests
 *   standard - Normal amounts (0.005 SOL, 5 REDACTED)
 *   stress   - Higher amounts (0.01 SOL, 10 REDACTED)
 *   large    - Big amounts (0.1 SOL, 10,000 REDACTED) for visible USD stats
 *
 * SAFETY:
...
// DemoREDACTED has 6 decimals, so 1_000_000n = 1 REDACTED
const AMOUNTS: Record<string, { sol: number; usdc: bigint }> = {
  small: { sol: 0.001, usdc: 1_000_000n },         // 0.001 SOL (~$0.10), 1 REDACTED
  standard: { sol: 0.005, usdc: 5_000_000n },      // 0.005 SOL (~$0.50), 5 REDACTED
  stress: { sol: 0.01, usdc: 10_000_000n },        // 0.01 SOL (~$1), 10 REDACTED
  large: { sol: 0.1, usdc: 10_000_000_000n },      // 0.1 SOL (~$10), 10,000 REDACTED
};
...
async function runEthereumDeposit(iteration: number): Promise<ExecutionResult> {
  ...
  try {
    ...
    if (balance < depositAmount) {
      throw new Error(`Insufficient DemoREDACTED: have ${Number(balance) / 1e6}, need ${Number(depositAmount) / 1e6}`);
    }
    ...
    const exec = createExecution({
      ...
      kind: 'deposit',
      venue: 'demo_vault',
      intent: `Deposit ${Number(depositAmount) / 1e6} DemoREDACTED to vault (test run ${iteration})`,
      action: 'deposit',
      ...
      usdEstimate: Number(depositAmount) / 1e6 * REDACTED_USD_PRICE,
...
async function runEthereumSwap(iteration: number): Promise<ExecutionResult> {
  ...
  try {
    ...
    if (balance < swapAmount) {
      throw new Error(`Insufficient DemoREDACTED: have ${Number(balance) / 1e6}, need ${Number(swapAmount) / 1e6}`);
    }
    ...
    const exec = createExecution({
      ...
      kind: 'swap',
      venue: 'demo_dex',
      intent: `Swap ${Number(swapAmount) / 1e6} DemoREDACTED for DemoWETH (test run ${iteration})`,
      action: 'swap',
      fromAddress: relayerAddress,
      ...
      usdEstimate: Number(swapAmount) / 1e6 * REDACTED_USD_PRICE,
      usdEstimateIsEstimate: true,
      relayerAddress: relayerAddress,
    });
    result.executionId = exec.id;
...
async function runSolanaMemo(iteration: number): Promise<ExecutionResult> {
  ...
  try {
    ...
    const exec = createExecution({
      ...
      kind: 'swap',
      venue: 'demo_dex',
      intent: `Swap ${swapAmount} SOL for REDACTED (proof memo, test run ${iteration})`,
      action: 'swap',
      fromAddress: senderPubkey,
      token: 'SOL->REDACTED',
    ...
    const memoText = `SWAP:${swapAmount}SOL->REDACTED:demo_dex:${Date.now()}:run${iteration}`;
...
async function runBridgeIntent(iteration: number): Promise<ExecutionResult> {
  ...
  try {
    ...
    const exec = createExecution({
      ...
      kind: 'bridge',
      venue: 'bridge_intent',
      intent: `Bridge intent: ${bridgeUsd.toLocaleString()} REDACTED Sepolia -> Solana (test run ${iteration})`,
      action: 'bridge',
      fromAddress: relayerAddress,
      token: 'REDACTED',
      amountUnits: (bridgeUsd * 1_000_000).toString(),
      amountDisplay: `${bridgeUsd.toLocaleString()} REDACTED`,
    ...
    const bridgeMetadata = `bridge_intent:${bridgeIntentId}:REDACTED:${bridgeUsd}:sepolia->devnet`;
    ...
    if (SOLANA_PRIVATE_KEY) {
      ...
      const memoProgramId = base58Decode('MemoSq4gqABAXKb96qnH8TysNcWxMyWCqXgDLGmfcHr');
      const memoText = `BRIDGE_ACK:${bridgeIntentId}:${bridgeUsd}REDACTED:received`;
      const memoData = Buffer.from(memoText);
