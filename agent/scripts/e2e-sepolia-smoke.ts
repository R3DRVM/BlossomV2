...
async function main() {
  ...
  if (FULL_MODE) {
    ...
    if (!process.env.USDC_ADDRESS_SEPOLIA) requiredVars.push('USDC_ADDRESS_SEPOLIA');
        ...
        if (FULL_MODE && isConfigError) {
          ...
          if (preflight.notes && Array.isArray(preflight.notes)) {
            preflight.notes.forEach((note: string) => {
              ...
              if (note.includes('USDC_ADDRESS_SEPOLIA')) missingVars.push('USDC_ADDRESS_SEPOLIA');
      ...
      try {
        ...
        if (portfolio.balances) {
          ...
          if (portfolio.balances.usdc) {
            console.log(`  USDC: ${portfolio.balances.usdc.formatted || '0'}`);
          }
      ...
      const chatResponse = await fetchJson(`${BASE_URL}/api/chat`, {
        ...
        body: JSON.stringify({
          ...
          clientPortfolio: {
            ...
            balances: [
              { symbol: 'ETH', balanceUsd: 30 }, // Has ETH, no USDC/WETH
            ],
          },
        }),
      });
