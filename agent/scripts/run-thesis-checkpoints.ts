...
// ============================================
// CHECKPOINT 1: DemoREDACTED Vault Deposit
// ============================================
async function checkpoint1(): Promise<CheckpointResult> {
  console.log('\n═══════════════════════════════════════════════════════════');
  console.log('CHECKPOINT 1: Sepolia DemoREDACTED Vault Deposit');
  console.log('═══════════════════════════════════════════════════════════\n');

  const result: CheckpointResult = {
    checkpoint: 1,
    name: 'DemoREDACTED Vault Deposit',
    status: 'FAIL',
    txHashes: [],
  };

  if (dryRun) {
    console.log('[DRY RUN] Would execute DemoREDACTED deposit to vault');
    result.status = 'SKIP';
    return result;
  }

  const startTime = Date.now();
  const depositAmount = smallAmounts ? 1_000_000n : 10_000_000n; // 1 or 10 REDACTED (6 decimals)

  try {
    // Check DemoREDACTED balance
    ...
    console.log(`DemoREDACTED balance: ${Number(balance) / 1e6} REDACTED`);
    if (balance < depositAmount) {
      throw new Error(`Insufficient DemoREDACTED. Need ${Number(depositAmount) / 1e6}, have ${Number(balance) / 1e6}`);
    }

    // Step 1: Approve vault to spend DemoREDACTED
    console.log('\nStep 1: Approving DemoREDACTED for vault...');
    ...
    const usdEstimate = Number(depositAmount) / 1e6 * REDACTED_USD_PRICE;
    ...
    const exec = createExecution({
      ...
      kind: 'deposit',
      venue: 'demo_vault',
      intent: `Deposit ${Number(depositAmount) / 1e6} DemoREDACTED into Demo Vault`,
      action: 'deposit',
      fromAddress: relayerAddress,
      toAddress: DEMO_LEND_VAULT_ADDRESS,
      token: 'DemoREDACTED',
      amountUnits: depositAmount.toString(),
      amountDisplay: `${Number(depositAmount) / 1e6} REDACTED`,
      usdEstimate,
    ...
    console.log(`   Amount: ${Number(depositAmount) / 1e6} REDACTED (~$${usdEstimate.toFixed(2)})`);
...
async function checkpoint2(): Promise<CheckpointResult> {
  ...
  // Check Aave config
  const AAVE_REDACTED = process.env.AAVE_REDACTED_ADDRESS;
  const AAVE_POOL = process.env.AAVE_SEPOLIA_POOL_ADDRESS;

  if (!AAVE_REDACTED || !AAVE_POOL) {
    console.log('⏭️  SKIP: Aave addresses not configured');
    console.log('   Set AAVE_REDACTED_ADDRESS and AAVE_SEPOLIA_POOL_ADDRESS in agent/.env.local');
    result.status = 'SKIP';
    return result;
  }

  const startTime = Date.now();
  const depositAmount = smallAmounts ? 100_000n : 1_000_000n; // 0.1 or 1 REDACTED

  try {
    // Check Aave REDACTED balance
    const balance = await publicClient.readContract({
      address: AAVE_REDACTED as `0x${string}`,
      abi: ERC20_ABI,
      functionName: 'balanceOf',
      args: [relayerAddress],
    }) as bigint;

    console.log(`Aave REDACTED balance: ${Number(balance) / 1e6} REDACTED`);

    if (balance < depositAmount) {
      console.log('⏭️  SKIP: Insufficient Aave REDACTED for deposit');
      result.status = 'SKIP';
      return result;
    }
    ...
    const supplyTx = await walletClient.writeContract({
      ...
      abi: AAVE_POOL_ABI,
      functionName: 'supply',
      args: [AAVE_REDACTED as `0x${string}`, depositAmount, relayerAddress, 0],
    });
    ...
    const exec = createExecution({
      ...
      kind: 'deposit',
      venue: 'aave',
      intent: `Supply ${Number(depositAmount) / 1e6} REDACTED to Aave V3`,
      action: 'supply',
      fromAddress: relayerAddress,
      toAddress: AAVE_POOL,
      token: 'REDACTED',
...
async function checkpoint3(): Promise<CheckpointResult> {
  ...
  const startTime = Date.now();
  const swapAmount = smallAmounts ? 1_000_000n : 5_000_000n; // 1 or 5 REDACTED
  ...
  try {
    ...
    if (balance < swapAmount) {
      throw new Error(`Insufficient DemoREDACTED. Need ${Number(swapAmount) / 1e6}, have ${Number(balance) / 1e6}`);
    }
    ...
    // With 95% rate: 1 REDACTED = 0.95 * 10^12 WETH units
    ...
    const swapTx = await walletClient.writeContract({
      ...
      args: [{
        tokenIn: DEMO_REDACTED_ADDRESS as `0x${string}`,
    ...
    const usdEstimate = Number(swapAmount) / 1e6 * REDACTED_USD_PRICE;
    ...
    const exec = createExecution({
      ...
      kind: 'swap',
      venue: 'demo_dex',
      intent: `Swap ${Number(swapAmount) / 1e6} DemoREDACTED for DemoWETH`,
      action: 'swap',
      fromAddress: relayerAddress,
      toAddress: DEMO_SWAP_ROUTER_ADDRESS,
      token: 'DemoREDACTED->DemoWETH',
      amountUnits: swapAmount.toString(),
      amountDisplay: `${Number(swapAmount) / 1e6} REDACTED`,
      usdEstimate,
      usdEstimateIsEstimate: true,
      relayerAddress: relayerAddress,
    });
    ...
    console.log(`\n✅ CHECKPOINT 3 PASSED`);
    console.log(`   Execution ID: ${exec.id}`);
    console.log(`   Amount: ${Number(swapAmount) / 1e6} REDACTED (~$${usdEstimate.toFixed(2)})`);
    console.log(`   Latency: ${latencyMs}ms`);
...
async function checkpoint5(): Promise<CheckpointResult> {
  ...
  try {
    ...
    const memoText = `SWAP:${swapAmount}SOL->REDACTED:demo_dex:${Date.now()}`;
    ...
    const exec = createExecution({
      ...
      kind: 'swap',
      venue: 'demo_dex',
      intent: `Swap ${swapAmount} SOL for REDACTED (proof memo)`,
      action: 'swap',
      fromAddress: senderPubkey,
      token: 'SOL->REDACTED',
...
async function checkpoint6(): Promise<CheckpointResult> {
  ...
  try {
    ...
    const bridgeMetadata = `bridge_intent:${bridgeIntentId}:REDACTED:sepolia->devnet:10`;
    ...
    const usdEstimate = 10; // 10 REDACTED bridge intent
    ...
    const exec = createExecution({
      ...
      kind: 'bridge',
      venue: 'bridge_intent',
      intent: `Bridge intent: 10 REDACTED from Sepolia to Solana devnet (${bridgeIntentId})`,
      action: 'bridge',
      fromAddress: relayerAddress,
      token: 'REDACTED',
      amountUnits: '10000000',
      amountDisplay: '10 REDACTED',
      usdEstimate,
      usdEstimateIsEstimate: true,
      relayerAddress: relayerAddress,
    });
