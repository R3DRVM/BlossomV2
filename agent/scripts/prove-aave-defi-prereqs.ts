...
async function main() {
  ...
  if (!TEST_USER_ADDRESS || !TEST_TOKEN || !TEST_AMOUNT_UNITS) {
    ...
    if (!TEST_TOKEN) {
      console.error('     - TEST_TOKEN (REDACTED or WETH)');
      record('ENV-2', false, 'TEST_TOKEN not set', 'Set TEST_TOKEN=REDACTED or TEST_TOKEN=WETH');
    }
    if (!TEST_AMOUNT_UNITS) {
      console.error('     - TEST_AMOUNT_UNITS (base units)');
      record('ENV-3', false, 'TEST_AMOUNT_UNITS not set', 'Set TEST_AMOUNT_UNITS=1000000 (for 1 REDACTED)');
    }
    console.error('\n   Example: TEST_USER_ADDRESS=0x... TEST_TOKEN=REDACTED TEST_AMOUNT_UNITS=1000000 npm run prove:aave-defi:prereqs');
    process.exit(1);
  }
  ...
  try {
    const { ETH_TESTNET_RPC_URL, REDACTED_ADDRESS_SEPOLIA, WETH_ADDRESS_SEPOLIA } = await import('../src/config');
    ...
    if (TEST_TOKEN === 'REDACTED') {
      tokenAddress = REDACTED_ADDRESS_SEPOLIA!;
      decimals = 6;
    } else if (TEST_TOKEN === 'WETH') {
      tokenAddress = WETH_ADDRESS_SEPOLIA!;
      decimals = 18;
    } else {
      record('PREREQ-4', false, `Unsupported TEST_TOKEN: ${TEST_TOKEN}`, 'Use TEST_TOKEN=REDACTED or TEST_TOKEN=WETH');
      process.exit(1);
    }
  ...
  try {
    const { ETH_TESTNET_RPC_URL, EXECUTION_ROUTER_ADDRESS, REDACTED_ADDRESS_SEPOLIA, WETH_ADDRESS_SEPOLIA } = await import('../src/config');
    ...
    if (TEST_TOKEN === 'REDACTED') {
      ...
      record('PREREQ-5', false, `Unsupported TEST_TOKEN: ${TEST_TOKEN}`, 'Use TEST_TOKEN=REDACTED or TEST_TOKEN=WETH');
      process.exit(1);
    }
