...
const TEST_AMOUNT_UNITS = process.env.TEST_AMOUNT_UNITS; // e.g. "1000000" for 1 USDC (6 decimals)
...
async function main() {
  ...
  if (!TEST_USER_ADDRESS || !TEST_TOKEN || !TEST_AMOUNT_UNITS) {
    ...
    console.error('   Example: TEST_USER_ADDRESS=0x... TEST_TOKEN=USDC TEST_AMOUNT_UNITS=1000000 npm run prove:aave-defi:e2e-smoke');
    process.exit(1);
  }
  ...
  try {
    const { ETH_TESTNET_RPC_URL, EXECUTION_ROUTER_ADDRESS, AAVE_USDC_ADDRESS, AAVE_WETH_ADDRESS } = await import('../src/config');
    ...
    if (!ETH_TESTNET_RPC_URL || !EXECUTION_ROUTER_ADDRESS) {
      ...
      let decimals: number;
      if (TEST_TOKEN === 'USDC') {
        tokenAddress = AAVE_USDC_ADDRESS!;
        decimals = 6;
      } else if (TEST_TOKEN === 'WETH') {
        tokenAddress = AAVE_WETH_ADDRESS!;
        decimals = 18;
      } else {
        assert(false, 'E2E-3', `Unsupported TEST_TOKEN: ${TEST_TOKEN} (must be USDC or WETH)`);
        return;
      }
      ...
      if (balance < amount) {
        ...
        const decimals = TEST_TOKEN === 'USDC' ? 6 : 18;
  ...
  try {
    // Calculate amount in human units (e.g., "1" for 1 USDC)
    const decimals = TEST_TOKEN === 'USDC' ? 6 : 18;
    const amountHuman = (parseFloat(TEST_AMOUNT_UNITS) / Math.pow(10, decimals)).toString();

    const defiExecutionRequest = {
      kind: 'lend' as const,
      amount: amountHuman, // Human-readable amount (e.g., "1" for 1 USDC)
      amountUsd: parseFloat(TEST_AMOUNT_UNITS) / (TEST_TOKEN === 'USDC' ? 1e6 : 1e18),
      asset: TEST_TOKEN,
      protocol: 'Aave',
    };
