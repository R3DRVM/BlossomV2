...
const TOKEN = args['token'] || 'REDACTED';
const AMOUNT_UNITS = args['amount'] || args['amountUnits'] || '1000000'; // Default 1 REDACTED (6 decimals)
...
function getTokenAddress(symbol: string): string {
  const tokens: Record<string, string> = {
    'REDACTED': process.env.DEMO_REDACTED_ADDRESS || '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48',
    'WETH': process.env.DEMO_WETH_ADDRESS || '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2',
  };
  return tokens[symbol.toUpperCase()] || tokens['REDACTED'];
}
      ...
      body: JSON.stringify({
        ...
        plan: {
          actions: [
            {
              type: 0, // swap
              inputToken: getTokenAddress('REDACTED'),
