...
function generateHelpfulFallback(userMessage, portfolio) {
    ...
    if (lower.includes('money') || lower.includes('invest') || lower.includes('profit') || lower.includes('make') || lower.includes('grow')) {
        const usdcBalance = portfolio?.balances.find(b => b.symbol === 'USDC')?.balanceUsd || 0;
        if (usdcBalance > 0) {
...
async function applyDeterministicFallback(userMessage, isSwapPrompt, isDefiPrompt, isPerpPrompt = false, isEventPrompt = false) {
    ...
    if (isDefiPrompt) {
        ...
        if (structuredAllocMatch) {
            // Structured format: "Allocate amountUsd:"500" to protocol:"Aave V3" USDC yield"
            ...
            // FALLBACK: Natural language format: "deposit 500 USDC" or "park 10 usdc into yield"
    ...
    try {
        ...
        if (executionRequest) {
            ...
            if (executionRequest.kind === 'perp') {
                ...
                const apyPct = 5.0; // Default to Aave USDC 5% for demo
                serverDraftId = `draft-${uuidv4()}`;
                const draftStrategy = {
                    id: serverDraftId,
                    type: 'defi',
...
app.post('/api/execute/prepare', maybeCheckAccess, async (req, res) => {
    ...
    try {
        ...
        const { DEMO_USDC_ADDRESS, DEMO_WETH_ADDRESS, EXECUTION_ROUTER_ADDRESS } = await Promise.resolve().then(() => __importStar(require('../config')));
        ...
        res.json({
            ...
            demoTokens: DEMO_USDC_ADDRESS && DEMO_WETH_ADDRESS ? {
                DEMO_USDC: DEMO_USDC_ADDRESS,
...
app.post('/api/execute/relayed', maybeCheckAccess, async (req, res) => {
    ...
    try {
        ...
        const USDC_ADDRESS_SEPOLIA = guardConfig.USDC_ADDRESS_SEPOLIA;
        ...
        // Guard 4: Validate token allowlist (ETH/WETH/USDC only for MVP)
        // Include both standard Sepolia tokens AND Aave-specific tokens
        const { AAVE_USDC_ADDRESS, AAVE_WETH_ADDRESS } = await Promise.resolve().then(() => __importStar(require('../config')));
        const allowedTokens = new Set();
        if (WETH_ADDRESS_SEPOLIA) {
            allowedTokens.add(WETH_ADDRESS_SEPOLIA.toLowerCase());
        }
        if (USDC_ADDRESS_SEPOLIA) {
            allowedTokens.add(USDC_ADDRESS_SEPOLIA.toLowerCase());
        }
        // Add Aave-specific tokens for lending
        if (AAVE_USDC_ADDRESS) {
            allowedTokens.add(AAVE_USDC_ADDRESS.toLowerCase());
        }
...
app.get('/api/portfolio/eth_testnet', maybeCheckAccess, async (req, res) => {
    try {
        ...
        // Require config
        const { ETH_TESTNET_RPC_URL, USDC_ADDRESS_SEPOLIA, WETH_ADDRESS_SEPOLIA } = await Promise.resolve().then(() => __importStar(require('../config')));
        if (!ETH_TESTNET_RPC_URL) {
            return res.status(500).json({
                error: 'ETH_TESTNET_RPC_URL not configured',
            });
        ...
        // Fetch USDC balance
        const usdcBalance = await erc20_balanceOf(USDC_ADDRESS_SEPOLIA, userAddress);
...
app.get('/api/wallet/balances', maybeCheckAccess, async (req, res) => {
    try {
        ...
        // Get config
        const { EXECUTION_MODE, ETH_TESTNET_RPC_URL, ETH_TESTNET_CHAIN_ID, DEMO_USDC_ADDRESS, DEMO_WETH_ADDRESS } = await Promise.resolve().then(() => __importStar(require('../config')));
        ...
        if (DEMO_USDC_ADDRESS) {
            try {
                const { erc20_balanceOf } = await Promise.resolve().then(() => __importStar(require('../executors/erc20Rpc')));
                const balance = await erc20_balanceOf(DEMO_USDC_ADDRESS, address);
                tokens.push({
 ...
 * Mints demo tokens (USDC and WETH) to a user address
 * Only available in eth_testnet mode
 */
app.post('/api/demo/faucet', maybeCheckAccess, async (req, res) => {
    try {
        const { EXECUTION_MODE, DEMO_USDC_ADDRESS, DEMO_WETH_ADDRESS } = await Promise.resolve().then(() => __importStar(require('../config')));
