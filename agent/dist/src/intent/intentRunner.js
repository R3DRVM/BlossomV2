...
async function executePerpEthereum(intentId, parsed, route) {
    ...
    // Import config
    const { RELAYER_PRIVATE_KEY, ETH_TESTNET_RPC_URL, DEMO_PERP_ADAPTER_ADDRESS, DEMO_REDACTED_ADDRESS, EXECUTION_ROUTER_ADDRESS, ERC20_PULL_ADAPTER_ADDRESS, } = await Promise.resolve().then(() => __importStar(require('../config')));
