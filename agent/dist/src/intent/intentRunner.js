...
export function parseIntent(intentText) {
    ...
    if (longMatch || shortMatch) {
        ...
        return {
            ...
            action: side,
            amount,
            amountUnit: 'USDC', // Assume USDC for perp margin
            targetAsset: asset,
            leverage,
...
async function executePerpEthereum(intentId, parsed, route) {
    ...
    try {
        ...
        const execution = await createExecution({
            ...
            token: 'DEMO_USDC',
            amountDisplay: parsed.amount ? `${parsed.amount} USDC @ ${parsed.leverage}x` : undefined,
        ...
        // Calculate margin amount (default 100 DEMO_USDC if not specified)
        // DEMO_USDC has 6 decimals
        const marginAmount = parsed.amount
            ? BigInt(Math.floor(parseFloat(parsed.amount) * 1e6))
            : BigInt(100 * 1e6); // 100 USDC default
        ...
        // Or assume the relayer already has DEMO_USDC
        // Encode router call
        ...
        // First, approve DEMO_USDC to ExecutionRouter (if not already approved)
        ...
        if (allowance < marginAmount) {
            // Approve DemoPerpAdapter to spend DEMO_USDC
