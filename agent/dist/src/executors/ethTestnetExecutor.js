...
function executionRequestToIntent(executionRequest) {
    ...
    if (!config_1.REDACTED_ADDRESS_SEPOLIA || !config_1.WETH_ADDRESS_SEPOLIA) {
    ...
    const tokenInAddr = executionRequest.tokenIn === 'ETH'
        ...
        : executionRequest.tokenIn === 'WETH'
            ...
            : config_1.REDACTED_ADDRESS_SEPOLIA.toLowerCase();
...
async function prepareEthTestnetExecution(args) {
    ...
    const isDemoSwap = executionKind === 'demo_swap' && config_1.DEMO_REDACTED_ADDRESS && config_1.DEMO_WETH_ADDRESS;
    if (isDemoSwap && !executionRequest) {
        // Default to REDACTED → WETH demo swap
        executionIntent = 'swap_usdc_weth';
        fundingPolicy = 'require_tokenIn';
        requestAmountIn = (0, viem_1.parseUnits)('100', 6); // 100 REDACTED default
    }
    // Lending supply mode: detect from executionKind or executionRequest
    const isLendSupply = executionKind === 'lend_supply' && config_1.DEMO_REDACTED_ADDRESS && config_1.DEMO_LEND_VAULT_ADDRESS && config_1.DEMO_LEND_ADAPTER_ADDRESS;
    let lendAmount;
    ...
    if (needsFundingRoute) {
        ...
        if (!config_1.REDACTED_ADDRESS_SEPOLIA) {
        ...
        if (executionRequest.tokenOut === 'REDACTED') {
            const tokenOut = config_1.REDACTED_ADDRESS_SEPOLIA.toLowerCase();
        ...
        const useDemoTokens = !useRealExecution && config_1.DEMO_REDACTED_ADDRESS && config_1.DEMO_WETH_ADDRESS && (isDemoSwap ||
        ...
        let swapAdapter;
        let pullAdapter;
        if (useDemoTokens && config_1.DEMO_REDACTED_ADDRESS && config_1.DEMO_WETH_ADDRESS) {
            // Use demo tokens and adapters
            tokenIn = executionIntent === 'swap_usdc_weth'
                ? config_1.DEMO_REDACTED_ADDRESS.toLowerCase()
        ...
        const { AAVE_SEPOLIA_POOL_ADDRESS, AAVE_ADAPTER_ADDRESS, AAVE_REDACTED_ADDRESS, AAVE_WETH_ADDRESS, LENDING_EXECUTION_MODE, } = await Promise.resolve().then(() => __importStar(require('../config')));
        ...
        if (useRealAave) {
            ...
            try {
                ...
                if (isWethLend && AAVE_WETH_ADDRESS) {
                    ...
                    if (AAVE_REDACTED_ADDRESS && AAVE_REDACTED_ADDRESS !== config_1.DEMO_REDACTED_ADDRESS) {
                        usdcAsset = {
                            symbol: 'REDACTED',
                            address: AAVE_REDACTED_ADDRESS.toLowerCase(),
        ...
        if (!useAaveSepolia) {
            ...
            asset = config_1.DEMO_REDACTED_ADDRESS.toLowerCase();
