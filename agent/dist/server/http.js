...
async function applyDeterministicFallback(userMessage, isSwapPrompt, isDefiPrompt, isPerpPrompt = false, isEventPrompt = false) {
    ...
    if (isDefiPrompt) {
        ...
        return {
            assistantMessage: `I'll park ${amount} USDC into yield. Recommended: ${vault?.name || 'Aave USDC'} at ${vault?.apy.toFixed(2) || '5.00'}% APY.`,
            actions: [],
            executionRequest: {
                kind: 'lend_supply',
                chain: 'sepolia',
...
app.post('/api/execute/relayed', maybeCheckAccess, async (req, res) => {
    ...
    try {
        ...
        const { EXECUTION_ROUTER_ADDRESS, UNISWAP_V3_ADAPTER_ADDRESS, WETH_WRAP_ADAPTER_ADDRESS, MOCK_SWAP_ADAPTER_ADDRESS, USDC_ADDRESS_SEPOLIA, WETH_ADDRESS_SEPOLIA, } = await import('../config');
