...
var init_defiLlamaQuote = __esm({
  "agent/src/quotes/defiLlamaQuote.ts"() {
    ...
    FALLBACK_VAULTS = [
      { name: "Aave USDC", apy: 5, tvl: 1e6, poolId: "demo-aave-usdc", protocol: "Aave" },
      { name: "Compound USDC", apy: 4.5, tvl: 8e5, poolId: "demo-compound-usdc", protocol: "Compound" },
...
__export(config_exports, {
  ...
  USDC_ADDRESS_SEPOLIA: () => USDC_ADDRESS_SEPOLIA,
  V1_DEMO: () => V1_DEMO,
...
var init_config = __esm({
  "agent/src/config.ts"() {
    ...
    USDC_ADDRESS_SEPOLIA = process.env.USDC_ADDRESS_SEPOLIA;
    WETH_ADDRESS_SEPOLIA = process.env.WETH_ADDRESS_SEPOLIA;
    DEMO_USDC_ADDRESS = process.env.DEMO_USDC_ADDRESS;
    ...
    AAVE_USDC_ADDRESS = process.env.AAVE_USDC_ADDRESS || DEMO_USDC_ADDRESS;
...
`;
  ...
  if (portfolio2) {
    ...
    const usdc = portfolio2.balances.find((b) => b.symbol === "USDC")?.balanceUsd || 0;
...
async function getDemoSwapQuote(params) {
  ...
  let expectedOut;
  if (tokenIn.toLowerCase() === DEMO_USDC_ADDRESS?.toLowerCase() && tokenOut.toLowerCase() === DEMO_WETH_ADDRESS?.toLowerCase()) {
    expectedOut = amountInBigInt * RATE_NUMERATOR / RATE_DENOMINATOR * 10n ** 12n;
  } else if (tokenIn.toLowerCase() === DEMO_WETH_ADDRESS?.toLowerCase() && tokenOut.toLowerCase() === DEMO_USDC_ADDRESS?.toLowerCase()) {
    expectedOut = amountInBigInt * RATE_NUMERATOR / RATE_DENOMINATOR / 10n ** 12n;
  } else {
...
async function getSwapQuote2(params) {
  const isDemoSwap = (params.tokenIn.toLowerCase() === DEMO_USDC_ADDRESS?.toLowerCase() || params.tokenIn.toLowerCase() === DEMO_WETH_ADDRESS?.toLowerCase()) && (params.tokenOut.toLowerCase() === DEMO_USDC_ADDRESS?.toLowerCase() || params.tokenOut.toLowerCase() === DEMO_WETH_ADDRESS?.toLowerCase());
    ...
    AAVE_V3_SEPOLIA_CONFIG = {
      ...
      supportedAssets: [
        ...
        // The actual USDC address on Sepolia may vary - this will be overridden by AAVE_USDC_ADDRESS if set
        {
          ...
          // Sepolia USDC testnet token (fallback)
...
function executionRequestToIntent(executionRequest) {
  ...
  const tokenInAddr = executionRequest.tokenIn === "ETH" ? "ETH" : executionRequest.tokenIn === "WETH" ? WETH_ADDRESS_SEPOLIA.toLowerCase() : USDC_ADDRESS_SEPOLIA.toLowerCase();
  const tokenOutAddr = executionRequest.tokenOut === "WETH" ? WETH_ADDRESS_SEPOLIA.toLowerCase() : USDC_ADDRESS_SEPOLIA.toLowerCase();
  let executionIntent;
...
async function prepareEthTestnetExecution(args) {
  ...
  if (needsFundingRoute) {
    ...
    const useDemoTokens = !useRealExecution && DEMO_USDC_ADDRESS && DEMO_WETH_ADDRESS && (isDemoSwap || executionRequest && executionRequest.kind === "swap" && (executionRequest.tokenIn === "USDC" && executionRequest.tokenOut === "WETH" || executionRequest.tokenIn === "WETH" && executionRequest.tokenOut === "USDC"));
    let tokenIn;
    let tokenOut;
    let swapAdapter;
    let pullAdapter;
    if (useDemoTokens && DEMO_USDC_ADDRESS && DEMO_WETH_ADDRESS) {
      tokenIn = executionIntent === "swap_usdc_weth" ? DEMO_USDC_ADDRESS.toLowerCase() : DEMO_WETH_ADDRESS.toLowerCase();
      tokenOut = executionIntent === "swap_usdc_weth" ? DEMO_WETH_ADDRESS.toLowerCase() : DEMO_USDC_ADDRESS.toLowerCase();
      ...
      tokenIn = executionIntent === "swap_usdc_weth" ? USDC_ADDRESS_SEPOLIA.toLowerCase() : WETH_ADDRESS_SEPOLIA.toLowerCase();
      tokenOut = executionIntent === "swap_usdc_weth" ? WETH_ADDRESS_SEPOLIA.toLowerCase() : USDC_ADDRESS_SEPOLIA.toLowerCase();
    ...
    const {
      ...
      AAVE_USDC_ADDRESS: AAVE_USDC_ADDRESS2,
    ...
    if (useRealAave) {
      try {
        ...
        if (isWethLend && AAVE_WETH_ADDRESS3) {
          ...
          let usdcAsset = await getSupportedAsset2("USDC");
          if (AAVE_USDC_ADDRESS2 && AAVE_USDC_ADDRESS2 !== DEMO_USDC_ADDRESS) {
            usdcAsset = {
              symbol: "USDC",
              address: AAVE_USDC_ADDRESS2.toLowerCase(),
              aTokenAddress: "0x0000000000000000000000000000000000000000",
              decimals: 6
            };
          }
      ...
      try {
        ...
        if (balance > 0n) {
          ...
          positions.push({
            ...
            underlyingValueUsd: asset.symbol === "USDC" ? parseFloat(balanceFormatted) : void 0
...
async function mintDemoTokens(recipientAddress) {
  const {
    ...
    DEMO_USDC_ADDRESS: DEMO_USDC_ADDRESS2,
  ...
  if (!DEMO_USDC_ADDRESS2 || !DEMO_WETH_ADDRESS2) {
...
CREATE TABLE IF NOT EXISTS executions (
    ...
    token TEXT,                             -- Token symbol: SOL, ETH, WETH, USDC, etc.
...
CREATE TABLE IF NOT EXISTS positions (
    ...
    margin_units TEXT,                          -- Margin in base units (6 decimals for USDC)
    margin_display TEXT,                        -- Human-readable margin (e.g., "100 USDC")
...
async function handleBridgeIntent(intentId, parsed, route) {
  ...
  const quoteResult = await getLiFiQuote2({
    ...
    fromToken: parsed.amountUnit || "USDC",
    toToken: parsed.amountUnit || "USDC",
...
async function executePerpEthereum(intentId, parsed, route) {
  ...
  if (!DEMO_PERP_ADAPTER_ADDRESS2 || !DEMO_USDC_ADDRESS2 || !EXECUTION_ROUTER_ADDRESS2) {
    updateIntentStatus2(intentId, {
      ...
      errorMessage: "DemoPerpAdapter or DEMO_USDC not configured"
    });
    return {
      ok: false,
      intentId,
      ...
      error: {
        ...
        message: "DemoPerpAdapter or DEMO_USDC not configured"
      }
    };
  }
  try {
    ...
    const execution = createExecution3({
      ...
      token: "DEMO_USDC",
      amountDisplay: parsed.amount ? `${parsed.amount} USDC @ ${parsed.leverage}x` : void 0,
    ...
    if (balance < marginAmount) {
      ...
      updateIntentStatus2(intentId, {
        ...
        failureStage: "execute",
        errorCode: "INSUFFICIENT_BALANCE",
        errorMessage: "Insufficient DEMO_USDC balance for perp margin"
      });
      return {
        ok: false,
        intentId,
        ...
        error: {
          ...
          message: "Insufficient DEMO_USDC balance for perp margin"
        }
      };
    }
    ...
    if (receipt.status === "success") {
      ...
      createPosition3({
        ...
        margin_display: `${(Number(marginAmount) / 1e6).toFixed(2)} USDC`,
...
function generateHelpfulFallback(userMessage, portfolio2) {
  ...
  if (lower.includes("swap") || lower.includes("trade") || lower.includes("exchange") || lower.includes("convert")) {
    return "I'd be happy to help with a swap! What token would you like to swap, and how much? For example: 'Swap 10 USDC to WETH' or 'Swap 0.01 ETH to USDC'.";
  }
  if (lower.includes("yield") || lower.includes("earn") || lower.includes("apy") || lower.includes("interest") || lower.includes("stake")) {
    return "Looking for yield opportunities? I can help deploy your USDC into DeFi protocols. How much would you like to deposit? For example: 'Deposit 100 USDC into Aave'.";
  }
  ...
  if (lower.includes("money") || lower.includes("invest") || lower.includes("profit") || lower.includes("make") || lower.includes("grow")) {
    const usdcBalance = portfolio2?.balances.find((b) => b.symbol === "USDC")?.balanceUsd || 0;
    if (usdcBalance > 0) {
      return `I can help you put your $${usdcBalance.toLocaleString()} USDC to work! Here are your options:
...
async function applyDeterministicFallback(userMessage, isSwapPrompt, isDefiPrompt, isPerpPrompt = false, isEventPrompt = false) {
  ...
  if (isSwapPrompt) {
    ...
    if (amountMatch && tokenInMatch) {
      ...
      const tokenOut = tokenOutMatch ? tokenOutMatch[1].toUpperCase() === "ETH" ? "WETH" : tokenOutMatch[1].toUpperCase() : tokenIn === "USDC" ? "WETH" : "USDC";
      return {
  ...
  if (isDefiPrompt) {
    const structuredAllocMatch = userMessage.match(/allocate\s+amount(Usd|Pct):"?(\d+\.?\d*)"?\s+to\s+protocol:"?([^"]+?)"?(?:\s+USDC|\s+yield|$)/i);
    let amount;
    let vaultName;
    ...
    return {
      assistantMessage: `I'll allocate $${amount} to ${vaultName || "yield vault"}. ${vaultName ? `Earning ~5-7% APY.` : "Recommended: Aave USDC at 5.00% APY."}`,
      actions: [],
      executionRequest: {
        kind: "lend_supply",
        chain: "sepolia",
    ...
    if (executionRequest) {
      ...
      if (executionRequest.kind === "perp") {
        ...
        const draftStrategy = {
          ...
          type: "defi",
          status: "draft",
          protocol: lendReq.vault || lendReq.protocol || "Aave USDC",
          vault: lendReq.vault || "Aave USDC",
          depositUsd: amountUsd,
          apyPct,
          sourceText: userMessage.substring(0, 200),
          isClosed: false,
          ...
          market: lendReq.vault || lendReq.protocol || "Aave USDC",
          side: "long",
...
app.post("/api/execute/prepare", maybeCheckAccess, async (req, res) => {
  ...
  try {
    ...
    const { DEMO_USDC_ADDRESS: DEMO_USDC_ADDRESS2, DEMO_WETH_ADDRESS: DEMO_WETH_ADDRESS2, EXECUTION_ROUTER_ADDRESS: EXECUTION_ROUTER_ADDRESS2 } = await Promise.resolve().then(() => (init_config(), config_exports));
    ...
    res.json({
      ...
      demoTokens: DEMO_USDC_ADDRESS2 && DEMO_WETH_ADDRESS2 ? {
        DEMO_USDC: DEMO_USDC_ADDRESS2,
...
app.post("/api/execute/relayed", maybeCheckAccess, async (req, res) => {
  ...
  try {
    ...
    const USDC_ADDRESS_SEPOLIA2 = guardConfig.USDC_ADDRESS_SEPOLIA;
    ...
    const { AAVE_USDC_ADDRESS: AAVE_USDC_ADDRESS2, AAVE_WETH_ADDRESS: AAVE_WETH_ADDRESS3 } = await Promise.resolve().then(() => (init_config(), config_exports));
    const allowedTokens = /* @__PURE__ */ new Set();
    if (WETH_ADDRESS_SEPOLIA2) {
      allowedTokens.add(WETH_ADDRESS_SEPOLIA2.toLowerCase());
    }
    if (USDC_ADDRESS_SEPOLIA2) {
      allowedTokens.add(USDC_ADDRESS_SEPOLIA2.toLowerCase());
    }
    if (AAVE_USDC_ADDRESS2) {
      allowedTokens.add(AAVE_USDC_ADDRESS2.toLowerCase());
    }
...
app.get("/api/portfolio/eth_testnet", maybeCheckAccess, async (req, res) => {
  try {
    ...
    const { ETH_TESTNET_RPC_URL: ETH_TESTNET_RPC_URL2, USDC_ADDRESS_SEPOLIA: USDC_ADDRESS_SEPOLIA2, WETH_ADDRESS_SEPOLIA: WETH_ADDRESS_SEPOLIA2 } = await Promise.resolve().then(() => (init_config(), config_exports));
    if (!ETH_TESTNET_RPC_URL2) {
      return res.status(500).json({
        error: "ETH_TESTNET_RPC_URL not configured"
      });
    }
    if (!USDC_ADDRESS_SEPOLIA2 || !WETH_ADDRESS_SEPOLIA2) {
      return res.status(500).json({
        error: "USDC_ADDRESS_SEPOLIA and WETH_ADDRESS_SEPOLIA must be configured"
      });
    }
    ...
    const usdcBalance = await erc20_balanceOf2(USDC_ADDRESS_SEPOLIA2, userAddress);
...
app.get("/api/wallet/balances", maybeCheckAccess, async (req, res) => {
  try {
    ...
    if (DEMO_USDC_ADDRESS2) {
      try {
        const { erc20_balanceOf: erc20_balanceOf2 } = await Promise.resolve().then(() => (init_erc20Rpc(), erc20Rpc_exports));
        const balance = await erc20_balanceOf2(DEMO_USDC_ADDRESS2, address);
        tokens.push({
          address: DEMO_USDC_ADDRESS2,
          symbol: "USDC",
          decimals: 6,
        ...
        notes.push(`USDC balance fetch failed: ${e.message}`);
      }
    } else {
      notes.push("DEMO_USDC_ADDRESS not configured");
    }
...
app.post("/api/demo/faucet", maybeCheckAccess, async (req, res) => {
  try {
    const { EXECUTION_MODE: EXECUTION_MODE2, DEMO_USDC_ADDRESS: DEMO_USDC_ADDRESS2, DEMO_WETH_ADDRESS: DEMO_WETH_ADDRESS2 } = await Promise.resolve().then(() => (init_config(), config_exports));
    if (EXECUTION_MODE2 !== "eth_testnet") {
      return res.status(400).json({
    ...
    if (!DEMO_USDC_ADDRESS2 || !DEMO_WETH_ADDRESS2) {
      return res.status(500).json({
        ...
        message: "DEMO_USDC_ADDRESS and DEMO_WETH_ADDRESS must be set in .env.local"
      });
    }
