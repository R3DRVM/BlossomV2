...
export function upsertUser(address, notes) {
...
export function upsertSession(userAddress, sessionId, status, expiresAt) {
    const db = getDatabase();
    const id = randomUUID();
...
export function getLatestSession(userAddress) {
    const db = getDatabase();
    const stmt = db.prepare(`
...
export function updateExecutionByCorrelationId(correlationId, updates) {
    const db = getDatabase();
    const now = Math.floor(Date.now() / 1000);
    const sets = ['updated_at = ?'];
    const values = [now];
...
export function listExecutions(limit = 50, offset = 0) {
    const db = getDatabase();
    return db.prepare(`
...
export function getUsersWithSessionStatus() {
    const db = getDatabase();
    return db.prepare(`
 ...
 */
export function getDevnetStats(feeBps) {
    const db = getDatabase();
    const now = Math.floor(Date.now() / 1000);
 ...
 */
export function updateExecutionWithFee(id, amountUnits, feeBps) {
    if (!amountUnits)
        return;
    const db = getDatabase();
 ...
 */
export function getTrafficStats(windowHours = 24) {
    const db = getDatabase();
    const now = Math.floor(Date.now() / 1000);
 ...
 */
export function getRequestLogStats(runId) {
    const db = getDatabase();
 ...
 */
export function getRecentTxHashes(limit = 20) {
    const db = getDatabase();
 ...
 */
export function migrateAddFeeColumns() {
    const db = getDatabase();
 ...
 */
export function listRuns(limit = 5) {
    const db = getDatabase();
    ensureRunsTable();
    return db.prepare(`
    SELECT * FROM runs
