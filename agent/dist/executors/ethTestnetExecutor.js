...
export async function prepareEthTestnetExecution(args) {
    ...
    if (executionRequest && executionRequest.kind === 'lend') {
        ...
        lendAmount = parseUnits(amountStr, 6); // Assume REDACTED decimals
        fundingPolicy = 'require_tokenIn';
    }
    ...
    if (needsFundingRoute) {
        ...
        const useDemoTokens = !useRealExecution && DEMO_REDACTED_ADDRESS && DEMO_WETH_ADDRESS && (isDemoSwap ||
        ...
        const amount = lendAmount || parseUnits('100', 6); // Default 100 REDACTED
        const { LENDING_EXECUTION_MODE, AAVE_POOL_ADDRESS } = await import('../config');
        const useRealAave = LENDING_EXECUTION_MODE === 'real' && AAVE_POOL_ADDRESS;
        // Use real Aave if configured, else demo vault
        const asset = useRealAave ? REDACTED_ADDRESS_SEPOLIA.toLowerCase() : (DEMO_REDACTED_ADDRESS.toLowerCase());
        const vault = useRealAave ? AAVE_POOL_ADDRESS.toLowerCase() : (DEMO_LEND_VAULT_ADDRESS.toLowerCase());
        const lendAdapter = useRealAave ? (DEMO_LEND_ADAPTER_ADDRESS || DEMO_LEND_ADAPTER_ADDRESS).toLowerCase() : (DEMO_LEND_ADAPTER_ADDRESS.toLowerCase());
        ...
        try {
            ...
            routingMetadata = {
                venue: `Supply REDACTED to ${lendingRouting.protocol}`,
        ...
        summary = `Supply ${amountDisplay} REDACTED to ${lendingRouting?.protocol || 'DemoLendVault'} (Est APR: ${lendingRouting?.apr || '5.00'}%)`;
        ...
        if ((isPerpStrategy || isEventStrategy) && PROOF_ADAPTER_ADDRESS) {
            ...
            // Hash the intent
            const intentHash = keccak256(toBytes(intentPayload));
