...
export async function prepareEthTestnetExecution(args) {
    ...
    if (executionRequest && executionRequest.kind === 'lend') {
        ...
        lendAmount = parseUnits(amountStr, 6); // Assume USDC decimals
        fundingPolicy = 'require_tokenIn';
    }
    ...
    if (needsFundingRoute) {
        ...
        const amount = lendAmount || parseUnits('100', 6); // Default 100 USDC
        ...
        const asset = useRealAave ? USDC_ADDRESS_SEPOLIA.toLowerCase() : (DEMO_USDC_ADDRESS.toLowerCase());
        ...
        try {
            ...
            routingMetadata = {
                venue: `Supply USDC to ${lendingRouting.protocol}`,
        ...
        summary = `Supply ${amountDisplay} USDC to ${lendingRouting?.protocol || 'DemoLendVault'} (Est APR: ${lendingRouting?.apr || '5.00'}%)`;
