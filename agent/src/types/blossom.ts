...
export type BlossomExecutionRequest =
  | {
      ...
      tokenOut: "WETH" | "USDC";
      amountIn: string;      // REQUIRED: decimal string, e.g. "0.01" ETH or "10" USDC
  ...
  | {
      ...
      chain: "sepolia";
      asset: "USDC"; // For now, only USDC lending
      amount: string; // decimal string, e.g. "100" for 100 USDC
      protocol?: "demo" | "aave"; // default: demo
      vault?: string; // Vault name from DefiLlama (e.g., "Aave USDC")
    }
  | {
      kind: "event";
      chain: "sepolia";
