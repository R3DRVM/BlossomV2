...
export async function prepareEthTestnetExecution(
  ...
  if (executionRequest && executionRequest.kind === 'lend') {
    ...
    lendAsset = (executionRequest as any).asset?.toUpperCase() || 'USDC';
  ...
  if (needsFundingRoute) {
    ...
    const useDemoTokens = !useRealExecution && DEMO_USDC_ADDRESS && DEMO_WETH_ADDRESS && (
      isDemoSwap ||
    ...
    const requestedAsset = lendAsset || (executionRequest as any)?.asset?.toUpperCase() || 'USDC';
      ...
      try {
        ...
        if (isWethLend && AAVE_WETH_ADDRESS) {
          ...
          if (AAVE_USDC_ADDRESS && AAVE_USDC_ADDRESS !== DEMO_USDC_ADDRESS) {
            usdcAsset = {
              symbol: 'USDC',
              address: AAVE_USDC_ADDRESS.toLowerCase() as Address,
