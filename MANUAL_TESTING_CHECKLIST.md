...
- Sepolia USDC/WETH tokens for swap testing
...
USDC_ADDRESS_SEPOLIA=0x94a9D9AC831246580d0C5B0C5c8b0b0C5B0C5B0C  # Verify on Sepolia
...
- Verify `USDC_ADDRESS_SEPOLIA` and `WETH_ADDRESS_SEPOLIA` on Sepolia Etherscan
...
**Note**: 
- `swap_usdc_weth` = swap USDC → WETH (you need USDC)
- `swap_weth_usdc` = swap WETH → USDC (you need WETH)
...
- Right panel or portfolio section should show:
  ...
  - **USDC balance** (matches MetaMask, if you have USDC)
...
**For `swap_usdc_weth`**:
- You need **Sepolia USDC** in your wallet
...
**In Chat UI**:
1. Type any message that creates a draft plan (e.g., "Swap 10 USDC for WETH")
...
1. **MetaMask popup #1**: "Approve USDC spending cap"
...
- Portfolio balances update (USDC decreases, WETH increases)
...
- Ensure your wallet has **no USDC** (or very little)
...
**Expected**:
- **No wallet popups**
- Chat shows error message: "Insufficient balance: You need X USDC but only have Y"
...
**Setup**:
1. In MetaMask, go to token (USDC or WETH)
...
- ETH, USDC, WETH balances from chain
...
6. **Add multi-hop swaps** (USDC → ETH → WETH)
