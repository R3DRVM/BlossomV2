...
Blossom is an **intelligent execution layer API** that routes trades across fragmented liquidity venues, settles in USDC, and connects to global fiat on/off ramps. Any wallet or app integrates with one API key and gets smart routing, intent processing, and cross-chain settlement.
...
- **USDC Settlement**: All trades settle in USDC for simplified accounting
...
An intelligent execution layer on **Soroban (Stellar)** that routes trades across fragmented liquidity venues, settles in USDC on Stellar, and connects to Stellar's anchor network for global fiat on/off ramps.
...
graph TB
    ...
    subgraph "Execution Layer (Soroban)"
        ...
        SETTLE[Settlement Contract<br/>USDC Handler]
    end
    ...
    subgraph "Settlement & Fiat"
        USDC[USDC on Stellar<br/>Circle Native]
...
├─────────────────────────────────────────────────────────────┤
│  Settlement Contract     │ USDC settlement handler          │
...
│                    Settlement Layer                          │
├─────────────────────────────────────────────────────────────┤
│  USDC (Circle Native)    │ Primary settlement asset         │
...
#### 4. **USDC (Circle Native)**
Primary settlement asset on Stellar.

```rust
// All settlements convert to USDC
...
impl SettlementContract {
    /// Handle USDC settlement and anchor integration
    pub fn settle(
        env: Env,
        user: Address,
        amount: i128,
  ...
  positions: [
    {
      ...
      market: "BTC-PERP",
      side: "long",
      margin_units: "100000000",  // 100 USDC (6 decimals)
      leverage: 10,
...
│ Step 1: User Intent                                          │
└─────────────────────────────────────────────────────────────┘
User: "Swap 100 USDC for ETH with max 0.5% slippage"
   │
   ▼
┌─────────────────────────────────────────────────────────────┐
...
- Check user balance (100 USDC available?)
...
ExecutionRouter.execute(plan)
   ...
   │   Transfer 100 USDC from user to router
   │
   ├─> Adapter 2: UniswapV3Adapter
   │   Swap 100 USDC → 0.027 ETH
   │
   └─> Return ETH to user
   │
...
struct Session {
    ...
    uint256 maxSpend;       // Max ETH/USDC per session (10 ETH default)
...
```json
{
  "message": "Swap 100 USDC for ETH with max 0.5% slippage",
  "chatId": "chat-123",
  "userAddress": "0x..."
}
```
...
```json
{
  "reply": "I can help you swap 100 USDC for ETH...",
  "planCard": {
    "action": "swap",
    "fromAsset": "USDC",
    "toAsset": "ETH",
...
const sendRemittance = async (
  ...
  // 1. Convert user's tokens to USDC on Stellar
  const swapIntent = `Swap ${amount} ${currency} to USDC`;
  const swapDraft = await blossom.processIntent(fromAddress, swapIntent);
  await blossom.execute(swapDraft);

  // 2. Route USDC through anchor for fiat withdrawal
  const withdrawIntent = `Send ${amount} USDC to ${recipientPhone} via anchor`;
...
- [ ] SEP-24 anchor integration for USDC on/off ramps
