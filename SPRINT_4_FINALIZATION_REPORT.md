...
**Missing**: Sepolia REDACTED balance >= `TEST_AMOUNT_UNITS`
...
❌ FAIL: ENV-2 - TEST_TOKEN not set
   Action: Set TEST_TOKEN=REDACTED or TEST_TOKEN=WETH
     - TEST_AMOUNT_UNITS (base units)
❌ FAIL: ENV-3 - TEST_AMOUNT_UNITS not set
   Action: Set TEST_AMOUNT_UNITS=1000000 (for 1 REDACTED)
...
- [ ] Fund test wallet with Sepolia REDACTED (at least 1 REDACTED = 1000000 units)
- [ ] Approve ExecutionRouter to spend tokens
- [ ] Create session via UI (enable one-click execution)

#### Phase 5: Run Real Proof
- [ ] Run: `TEST_USER_ADDRESS=0x... TEST_TOKEN=REDACTED TEST_AMOUNT_UNITS=1000000 npm run prove:real`
...
3. **Wallet must be funded**: Real execution requires Sepolia REDACTED
