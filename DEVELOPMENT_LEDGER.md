...
| **From** | EmKd4Z3HDUgjU2sYdCh9mEphMARUeNhKYtijw2Luz5rZ |
| **Slot** | 437273331 |
| **Latency** | 2474ms |
...
# https://faucet.solana.com/?address=EmKd4Z3HDUgjU2sYdCh9mEphMARUeNhKYtijw2Luz5rZ
...
SOLANA_DEVNET_PUBKEY=EmKd4Z3HDUgjU2sYdCh9mEphMARUeNhKYtijw2Luz5rZ
...
curl -s https://api.devnet.solana.com -X POST -H "Content-Type: application/json" \
  -d '{"jsonrpc":"2.0","id":1,"method":"getBalance","params":["EmKd4Z3HDUgjU2sYdCh9mEphMARUeNhKYtijw2Luz5rZ"]}' | jq '.result.value / 1e9'
```
...
| Mode | SOL Amount | USDC Amount | Use Case |
|------|------------|-------------|----------|
...
|------------|--------|--------------|----------------|
| 0: Checkpoint 0 | SKIP | - | - |
| 1: DemoUSDC Vault Deposit | PASS | 1b38c9d6 | [ethereum](https://sepolia.etherscan.io/tx/0xf7c9960c34ff97b4a64b807ed6cd1fa8e18aabce7fc0ce9ee174ffd2ea3d0d60), [ethereum](https://sepolia.etherscan.io/tx/0xbb74a53c0ec7df846b10c61984904f2f494529685f5bd440402582bca1bebf10) |
| 2: Checkpoint 2 | SKIP | - | - |
| 3: Checkpoint 3 | SKIP | - | - |
| 4: Checkpoint 4 | SKIP | - | - |
| 5: Checkpoint 5 | SKIP | - | - |
...
| 1: DemoUSDC Vault Deposit | PASS | 5e2a71f0 | [ethereum](https://sepolia.etherscan.io/tx/0x5acc696b853bde2ba3d8dff4b176785bfcc0448cca638d6fc79eef285269746e), [ethereum](https://sepolia.etherscan.io/tx/0xbba3bb790d11e347a4f97e51dae0e84072bf9cf9231e108a64316c30e9ec385d) |
| 2: Aave Deposit | SKIP | - | - |
...
| Pattern | Example |
|---------|---------|
| Swap | "swap 0.001 ETH to USDC" |
| Deposit | "deposit 100 USDC to vault" |
| Bridge | "bridge 50 USDC from eth to sol" |
...
3. Enter an intent (e.g., "swap 0.001 ETH to USDC")
...
| # | Intent | Expected |
|---|--------|----------|
| 1 | "swap 0.001 ETH to USDC" | Routed to demo_dex, TX on Sepolia |
| 2 | "deposit 1 USDC to vault" | Routed to demo_vault, TX on Sepolia |
...
2. **Bridge Flow**:
   ...
   curl -X POST http://localhost:3001/api/ledger/intents/execute \
     -H "Content-Type: application/json" \
     -H "X-Ledger-Secret: $LEDGER_SECRET" \
     -d '{"intentText": "bridge 100 USDC from ethereum to solana", "chain": "both"}'
...
1. User sends perp intent (e.g., "long ETH 2x with 100 USDC")
...
# Example: "long ETH 2x with 100 USDC"
...
curl -X POST http://localhost:3001/api/ledger/intents/execute \
  ...
  -d '{"intentText": "long ETH 2x with 100 USDC", "planOnly": true}' | jq '.'
