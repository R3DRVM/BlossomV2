...
- `TEST_AMOUNT_UNITS` - Amount in token units (e.g., `1000000` for 1 USDC with 6 decimals, or wei for WETH)
...
**Example:**
```bash
TEST_USER_ADDRESS=0x1234... TEST_TOKEN=USDC TEST_AMOUNT_UNITS=1000000 npm run prove:aave-defi:e2e-smoke
```
...
- Or use a Sepolia USDC faucet if available
- Or deploy/mint test USDC tokens if you have a testnet deployment
...
cast send <USDC_ADDRESS> "approve(address,uint256)" <EXECUTION_ROUTER_ADDRESS> <AMOUNT> \
