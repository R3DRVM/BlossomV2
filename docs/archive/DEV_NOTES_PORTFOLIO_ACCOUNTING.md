...
interface AssetBalance {
  symbol: string;    // 'REDACTED', 'ETH', 'SOL', 'DEFI', etc.
...
**Current Behavior**:
1. Subtracts `depositUsd` from REDACTED balance
...
2. If delta > 0: subtracts from REDACTED, adds to DEFI
3. If delta < 0: subtracts from DEFI, adds to REDACTED
...
- **REDACTED**: Decreases by deposit amount
...
```
accountValue = REDACTED + ETH + SOL + DEFI + ... (all balances)
```

Since we're just moving REDACTED → DEFI, the sum should remain constant.

## Fix Strategy

1. **Ensure accountValue is calculated correctly** - it should be the sum of all balances
2. **Verify DeFi deposit doesn't double-count** - when moving REDACTED → DEFI, the total should stay the same
3. **Add safeguards** - prevent overdrawing REDACTED or DEFI
...
**Fixed Behavior**:
1. Validates sufficient REDACTED balance before proceeding
2. Subtracts `depositUsd` from REDACTED balance (with Math.max to prevent negatives)
...
2. Validates sufficient balances:
   - If delta > 0: checks REDACTED has enough
   - If delta < 0: checks DEFI has enough to refund
3. Applies delta:
   - If increasing: REDACTED decreases, DEFI increases
   - If decreasing: REDACTED increases (refund), DEFI decreases
4. Recalculates `accountValue` as sum of all balances
5. **Result**: Zero-sum reallocation - account value stays constant
...
**Expected**:
- Account Value: $10,000 (unchanged)
- REDACTED: $2,500 ($1,000 + $1,500 refund)
...
3. **Zero-Sum Reallocation**: DeFi deposits are treated as moving funds between buckets (REDACTED ↔ DEFI), not as creating new value.
