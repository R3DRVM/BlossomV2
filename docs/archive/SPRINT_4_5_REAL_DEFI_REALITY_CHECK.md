...
⏭️  SKIP: Required environment variables not set
   ...
   Example: TEST_USER_ADDRESS=0x... TEST_TOKEN=USDC TEST_AMOUNT_UNITS=1000000 npm run prove:aave-defi:e2e-smoke
```
...
1. Set env vars: `TEST_USER_ADDRESS=0x... TEST_TOKEN=USDC TEST_AMOUNT_UNITS=1000000`
...
2. **Token Addresses**: The script assumes `USDC_ADDRESS_SEPOLIA` and `WETH_ADDRESS_SEPOLIA` are configured. If using demo tokens, these may need to be adjusted or the script should be updated to support demo token addresses.
