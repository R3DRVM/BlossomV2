...
2. `USDC_ADDRESS_SEPOLIA` and `WETH_ADDRESS_SEPOLIA` set?
...
3. `USDC_ADDRESS_SEPOLIA` and `WETH_ADDRESS_SEPOLIA` set?
4. Backend logs show errors?

**Common errors:**
- `UNISWAP_V3_ADAPTER_ADDRESS not configured` → Export the env var
- `USDC_ADDRESS_SEPOLIA not configured` → Export the env var
