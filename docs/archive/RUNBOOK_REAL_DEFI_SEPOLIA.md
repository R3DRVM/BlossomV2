...
- Sepolia test tokens (REDACTED or WETH)
...
# Optional: Override REDACTED if using different testnet token
AAVE_REDACTED_ADDRESS=0x...  # Only if different from REDACTED_ADDRESS_SEPOLIA
```
...
2. Find REDACTED token contract: `0x1c7D4B196Cb0C7B01d743Fbc6116a902379C7238`
3. Call `approve(spender, amount)`:
   - `spender`: Your `EXECUTION_ROUTER_ADDRESS`
   - `amount`: At least `TEST_AMOUNT_UNITS` (e.g., `1000000` for 1 REDACTED)
...
**Verify allowance**:
```bash
cast call $REDACTED_ADDRESS_SEPOLIA \
...
### Optional
- `AAVE_REDACTED_ADDRESS` - Override REDACTED address if different
