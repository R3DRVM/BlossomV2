...
AAVE_REDACTED_ADDRESS=0x...  # Sepolia REDACTED address (if different from default)
```
...
2. **REDACTED Address**: The adapter uses the REDACTED address from `AAVE_REDACTED_ADDRESS` env var or falls back to default. Ensure the correct Sepolia REDACTED address is configured.
