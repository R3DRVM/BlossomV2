...
**Key Functions**:
- `openPerp(spec)` (line 35): Opens perp position, deducts USDC, creates `PerpPosition`
- `closePerp(id)` (line 93): Closes position, calculates PnL, credits USDC
...
**Key Functions**:
- `openDefiPosition(protocol, asset, amountUsd)` (line 35): Deposits into protocol, deducts USDC
- `closeDefiPosition(id)` (line 74): Withdraws with yield calculation, credits USDC
...
**Key Functions**:
- `openEventPosition(eventKey, side, stakeUsd, label)` (line 70): Opens event bet, deducts USDC
