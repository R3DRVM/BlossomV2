...
- **Wallet Card** (sticky at top):
  ...
  - Token holdings list (USDC, ETH, SOL, etc.)
...
- **Inline Controls**:
  - **Deposit**: Numeric input + Max button + Update button
    - Max button sets deposit to available USDC balance
...
- **DeFi Card Controls**:
  ...
  - **Max Button**: Sets deposit to available USDC
...
1. User sends message: "Park my idle USDC in yield"
...
4. **Change Deposit**:
   ...
   - Or click Max to use all available USDC
   - Click Update or press Enter
   - Balances update (USDC ↔ DEFI)
5. **Close Plan**:
   - Click Close button
...
#### `defi_plan` Intent
- **Triggers**: "Park my idle USDC in yield", "Deposit in Aave"
