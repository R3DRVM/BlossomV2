...
## Test 2: Demo Swap (USDC → WETH)

### Steps

1. Type in chat: **"Swap 100 USDC to WETH"**
...
5. If prompted for approval:
   - MetaMask shows "Approve DEMO_USDC" tx
   - Confirm in MetaMask
...
- Assistant message appears with:
  ```
  Routing decision: USDC → WETH via [1inch/deterministic]
...
## Test 3: Lending Supply (USDC to Vault)

### Steps

1. Type in chat: **"Supply 100 USDC to lending"**
...
5. If prompted for approval (first time only):
   - Confirm USDC approval in MetaMask
...
- Assistant message:
  ```
  Lending decision: Supply USDC to DemoLendVault
